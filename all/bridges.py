...
def bridge(privatekey, amount, RPC, chain_id, scan):
    ...
    try:
        ...
        if chain_id == 42170:
            gasLimit = web3.eth.estimate_gas({'to': Web3.toChecksumAddress(address_wallet), 'from': Web3.toChecksumAddress(address_wallet),'value': web3.to_wei(0.0001, 'ether')}) + random.randint(50000, 100000)
        else:
...
def orbiter_bridge(privatekey, from_chain, to_chain, AMOUNT_TO_BRIDGE, MIN_BALANCE):
    ...
    if AMOUNT_TO_BRIDGE < 1.10101:
        decimal.getcontext().prec = 17
    else:
        decimal.getcontext().prec = 18

    if AMOUNT_TO_BRIDGE > 0.01:
        ...
        cprint('AMOUNT_TO_BRIDGE less 0.2', 'yellow')
